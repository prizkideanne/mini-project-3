...
export default function SortDropdown({ onSelectSort }) {
  const sorts = [
      ...
      <Menu as="div" className="text-left">
        <div>
          <Menu.Button className="inline-flex w-full justify-center rounded-md bg-red-400 px-4 py-2 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
            Sort
            <BsChevronDown className="-mr-1 ml-2 h-5 w-5 text-red-200 hover:text-red-100" />
          </Menu.Button>
        </div>
        <Transition
          as={Fragment}
        ...
        >
          <Menu.Items className="absolute left-0 mt-2 w-20 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
